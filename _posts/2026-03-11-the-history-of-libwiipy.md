...
Before starting on my own library, I did look around to see what my options were. Unfortunately, the only properly usable Wii library is libWiiSharp, and I was absolutely not going to be working in C#. I did find an older Python library called Wii.py that libWiiSharp was based off of, but it was ancient and couple only run under Python 2. There was an attempt at updating it, but it didn't support newer than Python 3.4 and had been abandoned many years prior, so it looked like I was going to be on my own.
